...
public class F23_SwapPaperQuestionAsync_Tests
{
    ...
    public async Task SwapPaperQuestionAsync_UTCID08_SwapGlobalTrue_ShouldSwapGlobally()
    {
        ...
        _repoMock.Setup(r => r.GetQuestionByIdAsync(request.NewQuestionId, _ct))
                 .ReturnsAsync(newQuestion);
        _repoMock.Setup(r => r.SwapExamQuestionGloballyAsync(paper.ExamId!.Value, request.OldQuestionId, request.NewQuestionId, _ct))
                 .Returns(Task.CompletedTask);
        ...
        // Assert
        _repoMock.Verify(r => r.SwapExamQuestionGloballyAsync(paper.ExamId!.Value, request.OldQuestionId, request.NewQuestionId, _ct), Times.Once);
        _repoMock.Verify(r => r.SwapPaperQuestionAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<int>(), It.IsAny<CancellationToken>()), Times.Never);
        _repoMock.VerifyAll();
    }

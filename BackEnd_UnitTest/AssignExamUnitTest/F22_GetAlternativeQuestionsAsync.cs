...
public class F22_GetAlternativeQuestionsAsync_Tests
{
    ...
    // UTCID06 – Paper tồn tại nhưng paper.Exam = null -> InvalidOperationException
    [Fact(DisplayName = "GetAlternativeQuestionsAsync - UTCID06 - Paper.Exam = null -> InvalidOperationException")]
    [TestType("A")]
    public async Task GetAlternativeQuestionsAsync_UTCID06_PaperWithoutExam_ShouldThrowInvalidOperation()
    {
        int paperId = 1;
        int questionId = 100;
        ...
        _repoMock.Setup(r => r.GetPaperWithQuestionsAsync(paperId, _ct)).ReturnsAsync(paper);
        _repoMock.Setup(r => r.GetQuestionByIdAsync(questionId, _ct)).ReturnsAsync(oldQuestion);

        var ex = await Assert.ThrowsAsync<InvalidOperationException>(
            () => _service.GetAlternativeQuestionsAsync(paperId, questionId, _ct));

        Assert.Equal("Paper has no associated exam.", ex.Message);
        _repoMock.Verify(r => r.GetAlternativeQuestionsAsync(
            It.IsAny<int>(), It.IsAny<int>(), It.IsAny<int>(),
            It.IsAny<string[]>(), It.IsAny<List<int>>(), It.IsAny<CancellationToken>()), Times.Never);
    }
